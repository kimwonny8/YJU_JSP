...
<form action="./test_r.jsp" method="post" >
    디스플레이 레코드수 변경 :
...
<br>
<a href = "./test_r.jsp?currentPageNo=0&limitCnt=<%=limitCnt%>">[처음]</a>	
<%
	if(currentPageNo > 0){
%>
	<a href="./test_r.jsp?currentPageNo=<%=(currentPageNo-1)%>&limitCnt=<%=limitCnt%>">[이전]</a>		
<%		
	}else {
%>	
	[이전]
...
%>
		<a href="./test_r.jsp?currentPageNo=<%=i%>&limitCnt=<%=limitCnt%>">[<%=(i+1)%>]</a>
<%		
		}
	}
%>
...
%>
	<a href="./test_r.jsp?currentPageNo=<%=(currentPageNo+1) %>&limitCnt=<%=limitCnt%>">[다음]</a>
<%	
	}else{
%>	
	[다음]
