...
<head>
<meta charset="UTF-8">
<title>마이쇼핑몰(KJW59) - 맥주자료조회(R4,R41)</title>
</head>
<body>
	...
	<a href="<%=beerContDir%>/BeerController.be3?actionType=R4&currentPageNo=0">[FIRST]</a>
	<%
	if (currentPageNo > 0) {
	%>
	<a href="<%=beerContDir%>/BeerController.be3?actionType=R4&currentPageNo=<%=(currentPageNo-1)%>">[PRE]</a>
	<%
	} else {
	%>
	[PRE]
	...
	%>
	<a href="<%=beerContDir%>/BeerController.be3?actionType=R4&currentPageNo=<%=i%>">[<%=(i+1)%>]</a>
	<%
		}
	}
	%>
	...
	%>
	<a href="<%=beerContDir%>/BeerController.be3?actionType=R4&currentPageNo=<%=(currentPageNo+1)%>">[NXT]</a>
	<%
	} else {
	%>
	[NXT]
	...
	<a href="<%=beerContDir%>/BeerController.be3?actionType=R4&currentPageNo=<%=(bpiVO.getPageCnt()-1)%>">[END]</a>
