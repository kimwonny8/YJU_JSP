...
public class BeerImageDAO {
	...
	/*
	 * // 게시판의 모든 레코드를 반환 메소드 - R public ArrayList<BeerDTO> getBeerList() {
	 * ArrayList<BeerDTO> list = new ArrayList<BeerDTO>();
	 * 
	 * String SQL = "select * from beer"; try { pstmt = con.prepareStatement(SQL);
	 * ResultSet rs = pstmt.executeQuery();
	 * 
	 * while (rs.next()) { BeerDTO beer = new BeerDTO();
	 ...
	 * beer.setB_image(rs.getString("b_image")); } rs.close();
	 * 
	 * } catch (SQLException e) { e.printStackTrace(); } finally { disConnect(); }
	 * return list; }
	 * 
	 * // 게시판의 현재 페이지 레코드를 반환 메서드 - R4 p29 public ArrayList<BeerDTO>
	 * getBeerListForPage(BeerPageInfoVO bpiVO) { ArrayList<BeerDTO> list = new
	 * ArrayList<BeerDTO>();
	 * 
	 * String SQL = "select * from beer ORDER BY b_id limit ?,?"; String SQL2 =
	 * "select count(*) from beer";
	 * 
	 * ResultSet rs;
	 * 
	 * try { pstmt = con.prepareStatement(SQL2); rs = pstmt.executeQuery();
	 * 
	 * while (rs.next()) { bpiVO.setRecordCnt(rs.getInt(1)); } } catch (SQLException
	 * e) { e.printStackTrace(); }
	 * 
	 * bpiVO.adjPageInfo();
	 * 
	 * try { pstmt = con.prepareStatement(SQL); pstmt.setInt(1,
	 * bpiVO.getStartRecord()); pstmt.setInt(2, bpiVO.getLimitCnt());
	 * 
	 ...
	 * beer.setB_like(rs.getInt("b_like"));
	 * beer.setB_dislike(rs.getInt("b_dislike"));
	 * beer.setB_image(rs.getString("b_image")); list.add(beer); } } catch
	 * (SQLException e) { e.printStackTrace(); } finally { disConnect(); } return
	 * list;
	 * 
	 * }
	 * 
	 * // 주 키 b_id의 레코드를 반환 메소드 - R public BeerDTO getBeer(int b_id) { String SQL =
	 * "select * from beer where b_id=?"; BeerDTO beer = new BeerDTO();
	 * 
	 * try { pstmt = con.prepareStatement(SQL); pstmt.setInt(1, b_id); ResultSet rs
	 * = pstmt.executeQuery(); rs.next();
	 * 
	 * beer.setB_id(rs.getInt("b_id")); beer.setB_code(rs.getString("b_code"));
	 * beer.setB_category(rs.getString("b_category"));
	 * beer.setB_name(rs.getString("b_name"));
