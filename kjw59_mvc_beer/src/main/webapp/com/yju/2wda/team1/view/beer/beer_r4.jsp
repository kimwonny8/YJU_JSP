...
<a href="<%=beerContDir%>/BeerController.jsp?actionType=R4&currentPage=0">[FIRST]</a>
<%
	if(currentPageNo > 0){
%>
	<a href="<%=beerContDir%>/BeerController.jsp?actionType=R4&currentPage=<%=(currentPageNo-1)%>">[PRE]</a>
<%
	} else{
%>
	[PRE]
...
%>
			<a href="<%=beerContDir%>/BeerController.jsp?actionType=R4&currentPage=<%=i%>">[<%=(i+1)%>]</a>
<%			
		}
	}
%>
...
%>
	<a href="<%=beerContDir%>/BeerController.jsp?actionType=R4&currentPage=<%=(currentPageNo+1)%>">[NXT]</a>
<%
	} else {
%>	
		[NXT]
...
%>
<a href="<%=beerContDir%>/BeerController.jsp?actionType=R4&currentPage=<%=(bpiVO.getPageCnt()-1)%>">[END]</a>
