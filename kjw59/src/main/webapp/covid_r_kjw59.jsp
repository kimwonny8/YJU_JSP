...
<body>
	...
	<form action="./covid_r_kjw59.jsp" method="post">
		디스플레이 레코드수 변경 : <select name="limitCnt">
			<option disabled value="0" selected>전체</option>
	...
	<%
		...
		if (recordCnt % limitCnt != 0)
			pageCnt++; // 남아있는 게 더 있다면 다음 페이지 있어야함~!
 ...
 */
	...
	<br>
	<a href="./covid_r_kjw59.jsp?currentPageNo=0&limitCnt=<%=limitCnt%>">[처음]</a>
	<%
	if (currentPageNo > 0) {
	%>
	<a
		href="./covid_r_kjw59.jsp?currentPageNo=<%=(currentPageNo - 1)%>&limitCnt=<%=limitCnt%>">[이전]</a>
	<%
	} else {
	%>
	[이전]
	...
	%>
	<a
		href="./covid_r_kjw59.jsp?currentPageNo=<%=i%>&limitCnt=<%=limitCnt%>">[<%=(i + 1)%>]
	</a>
	<%
	}
	}
	...
	%>
	<a
		href="./covid_r_kjw59.jsp?currentPageNo=<%=(currentPageNo + 1)%>&limitCnt=<%=limitCnt%>">[다음]</a>
	<%
	} else {
	%>
	[다음]
	...
	%>
	<a
		href="./covid_r_kjw59.jsp?currentPageNo=<%=(pageCnt - 1)%>&limitCnt=<%=limitCnt%>">[마지막]</a>
	<br>
